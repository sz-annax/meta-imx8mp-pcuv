...
on_negotiation_needed(GstElement *element, gpointer user_data)
{
    ...
    else
    {
        ...
        // gst_webrtc_rtp_transceiver_set_direction(transceiver, GST_WEBRTC_RTP_TRANSCEIVER_DIRECTION_SENDONLY);
...
check_plugins(void)
{
    ...
    const gchar *needed[] = {"opus", "vpx", "nice", "webrtc", "dtls", "srtp", "rtpmanager", "videotestsrc", "audiotestsrc", NULL};
...
sigint_handler_sighandler(int signum)
{
    ...
    if (pipe1)
    {
        gst_element_send_event(pipe1, gst_event_new_eos());
    }
